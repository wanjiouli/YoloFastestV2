...
def translate_info(file_names: list, save_root: str, class_dict: dict, train_val='train'):
    ...
    save__txt_path = "./my_yolo_dataset" + '/' + train_val +'.txt'
    save___txt_path = open(save__txt_path,"w")
    save_txt_path = os.path.join(save_root, train_val)

    if os.path.exists(save_txt_path) is False:
        os.makedirs(save_txt_path)
    # save_images_path = os.path.join(save_root, train_val, "images")
    save_images_path = os.path.join(save_root, train_val)
    ...
    for file in tqdm(file_names, desc="translate {} file...".format(train_val)):
        ...
        save___txt_path.write('D:\Ai_Project\Yolo-FastestV2-main'+path_copy_to[1:])
        save___txt_path.write('\n')

        if os.path.exists(path_copy_to) is False:
            shutil.copyfile(img_path, path_copy_to)

    save___txt_path.close()


def create_class_names(class_dict: dict):
    keys = class_dict.keys()
    with open("./my_yolo_dataset/category.names", "w") as w:
...
def main():
    # read class_indict    label_json_path = './data/pascal_voc_classes.json'
